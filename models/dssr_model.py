...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        print(get_parameter_number(self.netG))
        if opt["dist"]:
    ...
    def optimize_parameters(self, step):
        ...
        details, outs , out= self.netG(self.var_L)
        self.fake_SR = out
        self.real_Bic=F.interpolate(self.var_L, scale_factor=self.scale, mode='bicubic', align_corners=False)
        #self.fake_ker = ker_maps[-1]

        total_loss = 0
        for ind in range(len(outs)):
            d_sr = self.cri_pix(outs[ind], self.real_H)

            d_detail = self.cri_pix(details[ind], (self.real_H-self.real_Bic))
            
            total_loss=total_loss+d_sr+self.alpha*d_detail

            self.log_dict["l_detail%d" % ind] = d_detail.item()
            self.log_dict["l_pix%d" % ind] = d_sr.item()
            self.log_dict["l_total%d" % ind] = total_loss.item()
