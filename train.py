...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if(opt["ifbicubic"]):
              LR_img=util.b_Bicubic(train_data["GT"],opt["scale"])
              ker_map=None
            else:
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    img_dir = os.path.join(opt["path"]["val_images"], img_name) # for lmdb
                    #img_dir = os.path.join(opt['path']['val_images'], str(current_step), '_', str(step)) #for image
                    util.mkdir(img_dir)
                    ...
                    if gt_img.shape[2] == 3:  # RGB image
                      ...
                      avg_psnr_y += util.calculate_psnr(
                ...
                logger_val.info(
                    "<epoch:{:3d}, iter:{:8,d}, psnr: {:.6f},psnr_y:{:.6f}".format(
